...
### Section 3: Presence of domestic animals in the protected area(s), the documentation of their health status, and the perceived threats of domestic animals to conservation goals according to local surveys
...
num_responses_dom_animals_in_pa_are_concern_strongly_agree_agree_not_recorded<-
  terrestrial_data %>% 
  filter(local==T) |>
  select(dom_animals_concern,
         dom_animals_in_pa, 
         ...
         dom_animal_health_status_recorded) %>% 
  filter(dom_animals_concern%in% c("Strongly agree", "Agree")) %>% 
  filter(dom_animals_recorded=="No") %>% 
  nrow()
...
`r as.character(english::as.english(num_responses_dom_animals_in_pa)) |> stringr::str_to_sentence()` protected area data managers responded that domestic animals are found in their corresponding protected areas (`r round(num_responses_dom_animals_in_pa/nrow(terrestrial_data[terrestrial_data$local==T,])*100,1)`%). Among them, `r num_responses_dom_animals_in_pa_are_recorded` reported that domestic animals were documented if observed during patrols (`r round(num_responses_dom_animals_in_pa_are_recorded/num_responses_dom_animals_in_pa*100,1)`%), of which only `r num_responses_dom_animals_in_pa_are_recorded_health_status` reported recording their health status (`r round(num_responses_dom_animals_in_pa_are_recorded_health_status/num_responses_dom_animals_in_pa_are_recorded*100,1)`%). Protected area data managers’ responses for domestic animals as a conservation concern are shown in Fig 5. 

`r as.character(english::as.english(num_responses_dom_animals_in_pa_are_concern_strongly_agree_agree_not_recorded)) |> stringr::str_to_sentence()` data managers out of `r num_responses_dom_animals_in_pa_are_concern_strongly_agree_agree` that either “agree” or “strongly agree” with domestic animals as a conservation concern reported that these animals are not documented if observed during ranger patrols (`r round(num_responses_dom_animals_in_pa_are_concern_strongly_agree_agree_not_recorded/num_responses_dom_animals_in_pa_are_concern_strongly_agree_agree*100,1)`%).
...
### Section 4: Health data storage practices according to local surveys
...
For the subset of protected area data managers reporting the documentation of either sick, injured, or dead wildlife (`r data_smart_sick`, `r data_smart_injured`, and `r data_smart_dead` responses, respectively), their data was often stored in a SMART database (Fig 6). 
...
`r if(dom_animal_health_status_in_smart/num_responses_dom_animals_in_pa_are_recorded_health_status==1){"All "}` data managers reporting the recording of health status in domestic animals responded that this information was stored in a SMART database.
