...
#> encrypt the protected area names

library(sodium)
# #
# # # Generate a key
# key <- sodium::keygen(seed = random(32))
# saveRDS(key, "data/key.RDS")
# 
# 
# # Encrypt and convert to hex
# encrypted <- purrr::map(dat_modified$protected_area, \(x) 
#                        data_encrypt(serialize(x, NULL), key))
# 
# saveRDS(encrypted, "data/encrypted_pa_names.RDS")
# 
# dat_modified$protected_area<-NA
# 
# write.csv(dat_modified, "data/survey_responses_encrypted.csv")



#> open dataset and  decrypt protected area names

key <- readRDS("data/key.RDS")
encrypted <- readRDS("data/encrypted_pa_names.RDS")
dat_modified <- read.csv("data/survey_responses_encrypted.csv")

# Decrypt 
dat_modified$protected_area <- map(encrypted, \(x) 
                                   unserialize(data_decrypt(x, key)))
...
wildlife_health_important_perc<-round(wildlife_health_important/responses*100,0)


sick_injured_wildlife_found<-
terrestrial_data |> 
  # filter(local==T) |>
  filter(sick_injured_found!="Never") |> 
  nrow()

sick_injured_wildlife_found_perc<-round(sick_injured_wildlife_found/responses*100,0)
...
dead_wildlife_found_perc<-round(dead_wildlife_found/responses*100,0)



sick_injured_wildlife_recorded<-
  terrestrial_data |> 
  # filter(local==T) |>
  filter(sick_wl_recorded=="Yes") |> 
  nrow()

sick_injured_wildlife_recorded_perc<-round(sick_injured_wildlife_recorded/sick_injured_wildlife_found*100,0)


dead_wildlife_recorded<-
  terrestrial_data |> 
  # filter(local==T) |>
  filter(dead_wl_recorded=="Yes") |> 
  nrow()

dead_wildlife_recorded_perc<-round(dead_wildlife_recorded/dead_wildlife_found*100,0)
...
100-round(num_responses_dom_animals_in_pa_are_recorded/num_responses_dom_animals_in_pa*100,0)
...
100-round(num_responses_dom_animals_in_pa_are_recorded_health_status/
        num_responses_dom_animals_in_pa_are_recorded*100,0)
...
Judson SD, Fischer R, Judson A, Munster VJ. 2016. Ecological contexts of index cases and spillover events of different Ebolaviruses. PLoS Pathogens 12:e1005780.
...
Leguia M et al. 2023. Highly pathogenic avian influenza A (H5N1) in marine mammals and seabirds in Peru. Nature Communications 14, 5489. https://doi.org/10.1038/s41467-023-41182-0

Leroy EM et al. 2004. Multiple Ebola virus transmission events and rapid decline of central African wildlife. Science 303:387–390.
...
Nicola M, Alsafi Z, Sohrabi C, Kerwan A, Al-Jabir A, Iosifidis C, Agha M, Agha R. 2020. The socio-economic implications of the coronavirus pandemic (COVID-19): A review. International Journal of Surgery 78:185–193. 10.1016/j.ijsu.2020.04.018
...
Whitfield J. 2003. Ape populations decimated by hunting and Ebola virus. Nature 422:551. https://doi.org/10.1038/422551a
